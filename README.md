...
`v0.3` 已完成专题范围、测试边界、来源复核、草稿 Topic 和条款覆盖矩阵，正在建立 ICT 业务连续性、备份与恢复的 Source、Provision 和 Requirement。来源复核见 [`DORA-ICT-CONTINUITY-SOURCES-2026-09-07-R1`](docs/sources/dora-ict-business-continuity-source-review.md)，专题边界见[业务连续性、备份与恢复专题说明](docs/dora/topics/ict-business-continuity-backup-recovery.md)。当前固定发布仍为 `v0.2.0`，建设中的内容不能作为固定发布使用。
