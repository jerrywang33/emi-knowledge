...
项目处于初始化阶段，目前已确认独立专家知识库定位和 DORA 第一阶段方向，完成 DORA 权威来源基线 R2 和 DORA 全景目录，并形成最小知识模型、JSON Schema 与三条 ICT 变更管理要求的工程候选示例。知识模型仍待仓库维护者逐项复核，仓库尚未发布正式知识版本。README 中列出的内容范围是建设目标，不表示相关知识已经正式发布、审查通过或适用于任何具体 EMI 机构、司法辖区或生产系统。
