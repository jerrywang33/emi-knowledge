...
当前来源收集和版本核对结果见 [DORA 权威来源基线](docs/sources/dora-source-baseline.md)。该文件仍待人工复核，不能作为正式知识版本使用。
...
项目处于初始化阶段，目前已确认独立专家知识库定位和 DORA 第一阶段方向，并形成了待人工复核的 DORA 来源基线草案。README 中列出的内容范围是建设目标，不表示相关知识已经正式发布、审查通过或适用于任何具体 EMI 机构、司法辖区或生产系统。
