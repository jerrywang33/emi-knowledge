...
它保存的不是一批彼此孤立的文档，而是一条能够持续维护和检查的知识链：
...
| **EMI Harness** | 按资源 ID、版本和摘要选择已批准内容，形成一次任务的 EMI Context。 | 自动采用“最新内容”，或者让一次运行修改知识来源。 |
...
```text
公开权威资料 ──→ EMI Knowledge 公共知识
                              ├──→ 人员阅读与评审
机构内部资料 ──→ 私有知识层 ─┼──→ Agent 或产品的受控输入
                              └──→ EMI Harness 的任务级 EMI Context
```
...
- 检索数据库、向量索引和缓存都是可重新生成的读取方式，不是知识事实来源。

## 与 EMI Harness 的关系

[EMI Harness](https://github.com/jerrywang33/emi-harness) 与 EMI Knowledge 是两个独立系统：

| 系统 | 负责 | 不负责 |
| --- | --- | --- |
| **EMI Knowledge** | 采集来源、组织知识、记录适用范围与确认状态、维护 Skills 和模板、审查并发布版本。 | 运行研发任务、授予工具权限、修改目标代码或判断一次交付是否完成。 |
| **EMI Harness** | 为具体任务选择已批准知识，锁定版本与摘要，控制 Agent、工具和审批，验证结果并保存证据。 | 维护完整法规知识，或者在运行中自行修改已经引用的知识。 |

两者的连接方式如下：

```text
EMI Knowledge 中已批准的版本
  → Harness Resource Registry 校验资源 ID、版本和摘要
  → Human Authority 确认具体任务的适用性和私有补充信息
  → ContextManifest 锁定本次任务实际使用的 EMI Context
  → EMI Harness 执行、验证并保存交付证据
```

Harness 不能默认读取仓库中的全部内容，也不能自动跟随最新版本。一次已经开始的运行继续使用它锁定的知识版本；采用更新后的知识必须经过影响分析，并创建新的任务上下文或运行。
...
第一阶段不追求建立完整的欧洲 EMI 法规库，也不先建设搜索引擎或复杂知识图谱。目标是用少量真实内容证明知识能够被正确整理、审查、发布和使用。

计划按以下顺序推进：

1. 将 EMI Harness v0.1 中已作为工程校准输入使用过的客户资金保护内容整理为第一条完整知识链，明确哪些内容可以直接复用，哪些需要重新建模。
2. 定义最小 Schema 和校验规则，覆盖来源、版本、适用范围、内容分类、确认状态和相互引用。
3. 从同一份内容生成可阅读文档和可供 Agent、产品消费的版本化文件。
4. 让 EMI Harness 按资源 ID、版本和摘要使用发布结果，并证明旧任务不受新版本影响。
5. 以 DORA ICT 变更管理作为第一个新增主题，验证知识模型能否覆盖法规、监管说明、技术控制、测试和证据。

第一阶段完成的判断标准是：任意一项已发布要求都能追溯到权威来源，明确适用和待确认边界，关联至少一个可执行控制、检查方式和证据要求，并能被 Harness 以固定版本重复加载。

## 技术选择

- 知识正文优先使用 Markdown，结构化元数据优先使用 YAML 或 JSON，并由 JSON Schema 校验。
- Git 保存版本、评审和变更历史；正式发布版本使用不可变标识和内容摘要。
- 校验、转换和发布工具使用 TypeScript，与 EMI Harness 保持一致，但知识内容不依赖特定 Agent Runtime。
- 初始阶段不引入数据库、向量数据库或 RAG 服务。真实使用证明需要后再增加，并确保索引可以从仓库内容重新生成。
- 不为规划中的能力预建空目录或占位代码。首条知识链和 Schema 确认后，再据此确定实际仓库结构。

## 当前状态

项目处于初始化阶段，目前只确认了定位、边界和第一阶段方向。README 中列出的内容范围是建设目标，不表示相关知识已经收录、审查或适用于任何具体 EMI 机构、司法辖区或生产系统。
