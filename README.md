...
项目已经完成 DORA 权威来源基线 R2、DORA 全景目录、v0.1 知识模型与 JSON Schema，以及包含 57 个对象的 ICT 变更管理完整知识链。当前正在将 Schema、引用、覆盖和发布检查固化为 TypeScript 工具，仓库尚未发布 `v0.1.0`。README 中列出的长期内容范围是建设目标，不表示相关知识已经正式发布、审查通过或适用于任何具体 EMI 机构、司法辖区或生产系统。
