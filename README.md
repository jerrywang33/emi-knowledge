...
截至 2026-09-08，已发布三个 DORA Topic；“支付处理与异常”“账务与对账”“客户与账户”三份业务说明已形成审阅稿，配套 26 个合成案例和各自的来源、作者检查记录。账务专题另完成 5 组数值复算，最近一次 `npm run check` 的 33 项测试通过。这些业务稿尚未获维护者批准，也未完成独立使用或真实系统验证，不进入 `v0.3.0` 发布。

当前先记录进度，暂缓新增知识内容，接下来转去 review EMI Harness；该 review 尚未进行。EMI Knowledge 保持独立知识库定位，切换工作不改变现有内容的确认状态。

恢复后建议先补齐关键法规来源，再完成第四份业务说明“电子货币与资金流转”，随后做四份说明的联合检查、集中审阅和独立使用。首个正式 EMI Topic 暂以“电子货币发行与赎回”为候选，具体范围另行确认。阶段记录、未决事项和完成条件见 [V1.0 后续安排](roadmap/v1.0/README.md#恢复后的安排)。
