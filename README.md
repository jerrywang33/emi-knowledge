...
当前来源收集和版本核对结果见 [DORA 权威来源基线](docs/sources/dora-source-baseline.md)。该基线已经人工复核，可作为 v0.1 后续知识建设的来源输入；具体机构适用性仍需单独判断。
...
项目处于初始化阶段，目前已确认独立专家知识库定位和 DORA 第一阶段方向，完成 DORA 权威来源基线，并形成待人工复核的 DORA 全景目录。README 中列出的内容范围是建设目标，不表示相关知识已经正式发布、审查通过或适用于任何具体 EMI 机构、司法辖区或生产系统。
