...
当前来源收集和版本核对结果见 [DORA 权威来源基线](docs/sources/dora-source-baseline.md)。`DORA-SOURCES-2026-08-31` 已经人工复核，可作为 v0.1 后续知识建设的来源输入。文件同时记录全景目录阶段新发现的 FIR/03 与 `EBA/GL/2025/02` 来源增量，该增量仍待人工复核；具体机构适用性仍需单独判断。

DORA 的正式结构、主题、条款、配套文件和 EMI 适用路径见 [DORA 全景目录](docs/dora/dora-landscape.md)。当前目录处于待人工复核状态。
...
项目处于初始化阶段，目前已确认独立专家知识库定位和 DORA 第一阶段方向，完成 DORA 权威来源基线，并形成待人工复核的 DORA 全景目录和来源增量。README 中列出的内容范围是建设目标，不表示相关知识已经正式发布、审查通过或适用于任何具体 EMI 机构、司法辖区或生产系统。
