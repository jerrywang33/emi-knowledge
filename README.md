...
先建设“支付处理与异常”。每份业务说明覆盖适用范围、概念与参与方、正常流程、资金与记录关系、异常处理、相关要求和待确认问题。具体状态、记账时点和处置方式依据已确认的产品、渠道与来源填写，区分通用说明、设计建议和已确认规则。

业务说明放在现有 [`docs/emi-payments/`](docs/emi-payments/README.md) 中，关联各领域知识对象；受控 Topic 继续保存在 `knowledge/topics/`。本次只确认组织方式和建设顺序，业务正文、产品与渠道范围及来源复核尚未完成，不改变当前固定发布。
