...
| 项目 | 值 |
| --- | --- |
| 目录标识 | `DORA-LANDSCAPE-2026-08-31` |
| 状态 | 已人工复核，供 v0.1 建设使用 |
| 来源基线 | [`DORA-SOURCES-2026-08-31-R2`](../sources/dora-source-baseline.md) |
| 获取日期 | 2026-08-31 |
| 复核日期 | 2026-08-31 |
| 复核角色 | 仓库维护者 |
| 司法辖区 | 欧盟、Malta |
...
| 位置 | 内容定位 | 当前状态 |
| --- | --- | --- |
| R3-1.1.1 | FIR/03 对支付服务机构和 EMI 的范围入口。 | 已确认来源 |
| R3-1.1.4(x) | 要求 Licence Holder 参考适用法律和监管要求，其中列出 DORA。 | 已确认来源 |
| R3-2.10.1 | ICT and Cybersecurity Risk 部分明确要求 Licence Holder comply with DORA。 | 已确认来源 |
| R3-2.10.2 | 引用 amended EBA/GL/2019/04，对应 `EBA/GL/2025/02`。修订后保留 Section 3.8、paragraphs 92-98 的 payment service user relationship management 内容。 | 已确认来源 |

FIR/03 在第 2 步人工复核后由全景目录识别，其来源增量现已完成人工复核。来源事实已经确认，具体机构和业务的适用结论仍需机构事实支持。
...
| ESA 和 EBA 指引 | 2 项已确认 ESA 联合指引映射到 Articles 11 和 32；`EBA/GL/2025/02` 已确认并单独记录修订关系。 | 其他 Q&A、指引和 2026 年监督材料尚未建立完整目录。 |
...
| Malta 国内层 | 已定位 S.L. 330.20、Cap. 376、FIR/01 和 FIR/03 的相关位置，并确认 FIR/03 与 `EBA/GL/2025/02` 的引用关系。 | 具体机构适用性在后续知识链中继续确认。 |
...
仓库维护者于 2026-08-31 确认：

- 本目录只承担主题和来源导航，第 3 步不提取全部监管要求，也不形成机构适用性结论。
- 未获 EMD2 Article 9(1) 豁免与已获该豁免 EMI 使用两条不同的 ICT 风险管理路径。
- ICT 变更管理的直接条款、上下游目录关联和完整、简化框架边界可以作为后续知识建模输入。
- 当前 Malta 国内层范围满足 v0.1 目录需要，具体机构适用性继续保持待确认。
- 第 8 节列出的覆盖和缺口清楚，可以进入知识模型与 Schema 设计。

本次确认完成 Roadmap 第 3 步。目录确认只覆盖来源定位、结构、关系和边界，不改变第 8 节记录的限制。
